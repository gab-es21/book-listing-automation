...
Once we have a real ISBN, we look it up on **Almedina** (a Portuguese bookstore's own site search - good coverage for small local-press/book-club editions; personal low-volume use only, honest self-identifying User-Agent, not for bulk scraping). Every lookup sleeps a random 0.5-1.5s before making the request - enforced inside the lookup function itself, not left to callers to remember, since a burst of requests (confirmed live: as few as ~4 within a couple of minutes, even from unrelated ad-hoc scripts hitting it around the same time) trips a rate-limit block, sometimes for a while.
