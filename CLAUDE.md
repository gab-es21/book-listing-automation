...
- Outbound Discord notifications (`discord_notify.py`) are a plain webhook (`DISCORD_WEBHOOK_URL` in `.env`, optional), not a bot — no token, no persistent process.
- Inbound photo intake (`discord_fetch.py`, `blt fetch-discord-photos`, optional) reads a dedicated channel via a bot token (`DISCORD_BOT_TOKEN`/`DISCORD_PHOTOS_CHANNEL_ID`) and Discord's plain REST API — no `discord.py`, no gateway/websocket, no persistent process. It's on-demand only, run manually like `blt extract`/`group-all`. Don't turn this (or the webhook above) into an always-running bot/listener without discussing it first — nothing here needs real-time or two-way control from Discord.
