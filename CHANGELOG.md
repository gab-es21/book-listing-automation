...
- Multi-platform tracking: the tool is no longer Vinted-only. Each book now tracks which of Vinted/OLX/Marketplace it's currently posted on (checkboxes on `/review`'s create form, defaulting to Vinted; editable later on `/stock`'s inline edit row), and `/stock` shows a badge per flagged platform. Cross-posting an existing stock item to another platform via the review flow's ISBN-duplicate-merge adds that platform rather than replacing the existing one(s). Marking a copy sold on a book posted to only one platform behaves exactly as before (one click, no prompt); a book posted to several platforms gets a platform dropdown next to "Marcar 1 vendido" so each individual sale (quantity can drop across several separate platforms) records which one it actually sold on, snapshotted onto that sale's own `Sale.platform`, independent of the book's current flags. This is local tracking only - no posting/sync integration with any of the three platforms.
