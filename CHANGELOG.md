...
- Multi-platform tracking: the tool is no longer Vinted-only. The set of marketplaces is config-driven (`platforms.json` at the project root - add/remove/reorder entries with a slug/name/domain/optional default-checked flag/optional icon override, no code change needed, layout comfortably fits up to ~5). Each book tracks which of them it's currently posted on (checkboxes on `/review`'s create form, defaulting to whichever platform(s) `platforms.json` marks `default: true`; editable later on `/stock`'s inline edit row), and `/stock` shows a badge per flagged platform, complete with each site's favicon fetched live client-side (`s2/favicons`, no logo files bundled/redistributed) with a graceful fallback to plain text if that fails to load. Cross-posting an existing stock item to another platform via the review flow's ISBN-duplicate-merge adds that platform rather than replacing the existing one(s). Marking a copy sold on a book posted to only one platform behaves exactly as before (one click, no prompt); a book posted to several platforms gets a platform dropdown next to "Marcar 1 vendido" so each individual sale (quantity can sell out across several different platforms) records which one it actually sold on, snapshotted onto that sale's own `Sale.platform`, independent of the book's current flags. This is local tracking only - no posting/sync integration with any marketplace.
- `blt fetch-discord-photos`: a faster alternative to a USB cable transfer for getting phone photos onto this machine - send them to a dedicated Discord channel instead, then pull them into `RAW_DIR` with one command. On-demand only (run manually, like `blt extract`/`group-all`), a plain authenticated REST call (no `discord.py`, no persistent bot/gateway connection). Each downloaded photo's mtime is set from its Discord message's own timestamp rather than left at download time, since Discord sometimes strips EXIF - `group_photos.py`'s chronological cover/ISBN pairing depends on getting this right. Successfully downloaded messages are deleted to keep the channel a clean inbox; a failed delete just retries next run instead of ever silently duplicating or losing a photo. Requires `DISCORD_BOT_TOKEN`/`DISCORD_PHOTOS_CHANNEL_ID` in `.env` (optional, disabled with a clear error when unset) and a bot invited to its own dedicated channel - see README for setup. A "Verificar Discord" button on `/raw` triggers the same fetch from the browser - new pairs show up automatically once it's done, no need to drop to the CLI.
- All three Discord buttons ("Enviar para Discord" on `/review`/`/stock`, "Verificar Discord" on `/raw`) now show Discord's own favicon instead of a generic icon, fetched the same live client-side way (`s2/favicons`) as the marketplace badges.
- `/sorted`'s "Detetar livros" now runs in the background with the same live progress bar/spinner/"a detetar X de Y" counter as `/review`'s "Procurar todos novamente", instead of leaving the page hanging for the whole paced multi-book extraction run. The `.bulk-progress` styling is now shared between both pages instead of duplicated.

### Fixed
- `.editorconfig` and `.gitattributes` were listed in `.gitignore` and had never actually been committed, despite being standard shared project config (not personal/local settings) - now tracked so line-ending/encoding rules apply for anyone cloning the repo. `.gitignore` also gained an explicit `blt.db.bak*` pattern (database backup files made before a migration weren't covered by the existing `*.db` rule) and an explicit `.mypy_cache/` entry (already self-protected via its own internal auto-generated gitignore, but now documented at the root too).
- The dashboard's cross-page progress bar (Raw/Ordenadas/Por confirmar/Stock) used 4 arbitrary colors with no shared meaning across the app. Now each stage reuses a color that already means the same thing elsewhere here: Stock is `var(--green)`, the same green `/stock`'s "Disponível" and `/review`'s "Pronto" already use for "done/good"; Por confirmar is the same amber `/review`'s own queue breakdown already uses for "Repetido" (needs your attention); Raw (violet) and Ordenadas (blue) are visually distinct from both and from each other. Two unrelated elements that incidentally shared these CSS variables (`/review`'s "Passar" button, the bulk re-search progress fill) were decoupled first so this change couldn't silently recolor them too. The same per-stage color now carries through everywhere that stage shows up: the sidebar's active-item highlight and per-item hover, each page's header underline, its header-action/secondary/primary buttons' hover state, the `/raw` pair-role labels and swap-checkbox accent, and the dashboard's flow-diagram icons/counts/hover borders - driven from one `--page-accent`/`--page-accent-dark` CSS variable pair set from `active_step` on `<body>`, rather than each page hardcoding its own copy. `/review`'s "Criar" button is a deliberate exception, kept green like every other "safe to click" action app-wide rather than following the page's amber accent.
